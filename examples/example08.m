...
%%
  if ( ~exist('No','var') )
    No     = 3;   % number of van der Pol oscillators
    m      = 2;
    degree = 3;
  end
  ...
  a  = ones(No,1);   % viscous damping parameter
  b  = ones(No,1);   % coupling parameter
  ...
  if ( m>2 )
    if ( No>3 )
      ...
      warning('about to set a nonexistent control')
    end
  end
  ...
  x0 = [0.3*ones(No,1), zeros(No,1)].';  x0 = x0(:);
